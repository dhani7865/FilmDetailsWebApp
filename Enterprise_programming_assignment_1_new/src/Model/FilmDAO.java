...
public class FilmDAO {
	 ...
	 *  object or a new film object with values for the different fields. 
	 */
	...
	public static Film getFilmOrDefault(int id) {
		...
		} // CLose if statement fo film == null
		// otherwise the film
		return(film);
	} // Close getFilmOrDefault method
	...
	public static Film getNamedFilm(int id, String title, int year, String director, String stars, String review) {
		// Creating collection of films and calling the get simple films method
		Collection<Film> films = getSampleFilms().values();
		// Creating cor loop for the films
		for(Film f: films) {
			//			if(Integer.toString(f.getId()(id)) &&
			//			if(f.getId() == (id) &&
			// If statement to return the films, depending on id which is sued
			if(f.getId() == id &&
		...
		} // CLose for loop for films
		// otherwise return null/nothing
		return(null);
	} // Close get named film method
